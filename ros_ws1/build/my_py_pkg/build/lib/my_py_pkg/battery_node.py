...
class BatteryNode(Node): #CHANGE NAME
    def __init__(self):
        super().__init__("battery_node") #CHANGE NAME
    ...
    def call_set_led_server(self, led_number, state):
        client = self.create_client(SetLedPanel, "set_led_panel")
        while not client.wait_for_service(1.0):
            self.get_logger().warn("Waiting for Server Led Status...")

        request = SetLedPanel.Request()
